...
export default {
  ...
  actions: {
    fetchItems: async ({ commit }) => {
      const response = await api.plots();
    ...
    addItem: async ({ commit }, { plot, price }) => {
      const item = await api.add({ plot,price });
      commit('addItem', item);

    },
    updateItem: async ({ commit }, { id, plot, price }) => {
      const item = await api.update({ id, plot, price });
